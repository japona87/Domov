...
import { sendTenantAccessOnboarding, resendTenantAccessOnboarding, getTenantPassword, updateTenantPassword } from '@/lib/actions/onboarding'

const SUBJECT = 'Domov — Bienvenido a tu portal de arrendatario'
...
export function TenantOnboardingButton({ tenantId, hasAccess }: { tenantId: string; hasAccess: boolean }) {
  ...
  async function handleSend() {
    ...
    try {
      const data = hasAccess
        ? await resendTenantAccessOnboarding(tenantId)
        : await sendTenantAccessOnboarding(tenantId)
      setResult(data)
    } catch (e) {
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4">
            <h3 className="text-lg font-semibold text-slate-800">
              {hasAccess ? 'Reenviar onboarding' : 'Onboarding arrendatario'}
            </h3>
            ...
            {!result && !error && (
              <>
                <p className="text-sm text-slate-600">
                  {hasAccess
                    ? 'Se reenviarán las credenciales al arrendatario.'
                    : 'Se creará un usuario, se asignará al arrendatario y se enviarán las credenciales.'}
                </p>
...
export function TenantPasswordManager({ tenantId, hasAccess }: { tenantId: string; hasAccess: boolean }) {
  ...
  async function loadPassword() {
    setLoading(true)
    try {
      const pwd = await getTenantPassword(tenantId)
  ...
  async function handleSave() {
    ...
    setLoading(true)
    try {
      await updateTenantPassword(tenantId, password)
...
function emailBody(data: { fullName: string; email: string; password: string }) {
  return [
    `¡Hola ${data.fullName}! 👋`,
    '',
    'Te damos la bienvenida a Domov.',
    '',
    'Desde tu portal podrás:',
    '  📄 Ver los detalles de tu contrato',
    '',
    '━━━ Tus datos de acceso ━━━',
    `  🔗 Portal:   https://domov.app/tenant/dashboard`,
    `  📧 Email:    ${data.email}`,
